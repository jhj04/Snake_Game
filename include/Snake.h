...
private:
    ...
    int* outGate(int ex, int ey);
public:
    Snake(GameMap& map, int startX, int startY); // 생성자
    int getHeadX() const; // !! 아이템 구현을 위해 getter 추가
    int getHeadY() const; // !!
    void grow(); // !! 뱀의 길이를 늘리는 함수
    void shrink(); // !! 뱀의 길이를 줄이는 함수
    bool isWithinRange(int x, int y) const; // 뱀의 머리 근처를 확인하는 함수 (뱀의 머리 기준 3*3칸에는 아이템x)
